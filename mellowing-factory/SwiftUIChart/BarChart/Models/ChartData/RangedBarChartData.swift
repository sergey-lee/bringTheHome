...
public final class RangedBarChartData: CTRangedBarChartDataProtocol, GetDataProtocol, Publishable, PointOfInterestProtocol {
    ...
    public final var average: Double {
        let upperDataPointsWithoutZeros = dataSets.dataPoints
            .map(\.upperValue)
            .filter { $0 > 0 }
        
        let upperAverage = upperDataPointsWithoutZeros
            .reduce(0, +)
            .divide(by: Double(upperDataPointsWithoutZeros.count))
        
        let lowerDataPointsWithoutZeros = dataSets.dataPoints
            .map(\.lowerValue)
            .filter { $0 > 0 }
        
        let lowerAverage = lowerDataPointsWithoutZeros
            .reduce(0, +)
            .divide(by: Double(lowerDataPointsWithoutZeros.count))
